...
def find_strongest_image(layer_num, top=9, folder='/home/z840/ALISURE/Data/VOC2012/ILSVRC2012_img_val'):
    """
    从 .csv 文件读入数据， 拷贝特征图上响应最大的图片
    :param layer_num: 
    ...
    # 根据每张特征图响应度大小给图片排序，获得下标
    argsorted_activations = activations.argsort(axis=0)
    # 对于每一张特征图， 选择数据集中响应最大的top张图片
    ...
    # 从原来的目录下，拷贝对每张特征图响应最大的top张图片到指定目录
    for f in range(1, filters + 1):
        for index, img_id in enumerate(top_indices[f]):
            img_name = str(img_id)
            while len(img_name) < 5:
                img_name = '0' + img_name
            copy_from = read_from_folder + '/ILSVRC2012_val_000' + img_name + '.JPEG'
            copy_to = "{}/Layer{}_Filter{}_Top{}.JPEG".format(save_to_folder, layer_num, f, index + 1)
