...
class Deconvolution:
    ...
    def project_down(self, image_path, layer, f=None, use_bias=False):
        ...
        if f:
            self._set_zero_except_maximum() # 设置响应最大值以外的值为0，每个特征层将只有一个值不为0
    ...
    def _project_through_split_convolution(self):
        ...
        # Apply transposed filters： 对前一半和后一半分别采用解卷积
    ...
    def _unpool(self):
        ...
        for f in range(output_shape[0]): # 对于每一张特征图
            for i_out in range(output_shape[1]): # 对于每一行
                for j_out in range(output_shape[2]): # 对于每一列
    ...
    # 设置最大值以外的值为0， 结果将是该层下只有一张特征图的一个点不为0
    def _set_zero_except_maximum(self):
        # 该层除了响应最大的特征图以外的特征图设置为0
...
def project_multiple_layer_filters(img_id=None, deconv_base_model=None):
    ...
    for layer in (5, ): # 1, 2, 3, 4,
        ...
        projection = deconv_base_model.project_down(path, layer, max_filter) # 某一层的特征图解卷积后的重构图

        if layer != 1:
            # 提升对比度
            percentile = 99
            # max_val = np.nanargmax(unarranged_array)
            max_val = np.percentile(projection, percentile) # 百分位数： 具体参见 stackflow.com/questions/2374640/how-do-i-calculate-percentiles-with-python-numpy
